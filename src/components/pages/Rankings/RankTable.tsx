...
type RankTableProps = {
  data: {
    data: TableData[]; // Atualizado para usar o tipo TableData[]
  };
};
        ...
        <tbody className='bg-neutral-800 divide-y divide-neutral-700'>
          {data.data.map((user) => (
            ...
            >
              ...
              <td className='px-6 py-4'>Lv.{user.level} - {user.exp_percent}%</td>
              <td className='px-6 py-4'>{Number(user.scores).toLocaleString()}</td>
              <td className='px-6 py-4'>{Number(user.top_score).toLocaleString()}</td>
              <td className='px-6 py-4 font-medium whitespace-nowrap text-white'>{Number(user.total_score).toLocaleString() ?? 1}</td>
            </tr>
          ))}
        </tbody>
      </table>
